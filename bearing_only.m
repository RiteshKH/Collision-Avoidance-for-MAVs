function xtrue = bearing_only(xc,yc,sp,ep,xtrue)
S = warning('OFF')
...
veh=[0 -0.3 -0.3; 0 0.2 -0.2]; % vehicle animation % [0 90 90; 0 -50 50]
% xtrue= [sp(1);sp(2);0];
left = 0;
dt= DT_CONTROLS;
...
output = initialise_store(xtrue);
 ...
 g_vec = [];
while(d > 0.01)
    if((xtrue(3)>2*pi/3)||xtrue(3)<-2*pi/3)
        xtrue(3) = pi_to_pi2(xtrue(3));
        left = 1;
    end
    ...
    if((isempty([x_i y_i])~=1)&&(flag~=1))
        ...
        if(left==0)
            ...
            ang1 = pi_to_pi2(atan2((y_i(1) - xtrue(2)),(x_i(1) - xtrue(1))));
            ang2 = pi_to_pi2(atan2((y_i(2) - xtrue(2)),(x_i(2) - xtrue(1))));
        end
        if (ang1<ang2)
            temp = ang1;
            ang1 = ang2;
